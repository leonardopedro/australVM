(*
   ...
   extensions (the authorization gate, `why3_plugin.ml`; the NPU DMA gate,
   `npu_dma_plugin.ml` — the GPU.md hardware-invariant pass) register as
   plugins of that compiler. The CLI boot sequence is therefore just "load
   the plugins": `Vm_plugin.boot` installs the built-in compiler (which
   installs the Why3 gates), and every compile request is routed through the
   registry (`run_compiler`) instead of a hard-coded pipeline call.
...
(** Install the built-in compiler plugin (idempotent). The built-in pipeline
    installs the Why3-derived passes (`Why3_plugin.install`,
    `Npu_dma_plugin.install`) as part of its boot, so the gates are plugins
