...
def zero_loop
  ...
  := do
  loop
    (fun (x1, i1) => zero_loop.body x1 i1)
    (x, i)
...
/-- [tutorial::add_no_overflow]: loop body 0:
    Source: 'src/lib.rs', lines 177:4-180:5
    Visibility: public -/
@[rust_loop_body]
def add_no_overflow_loop.body
...
def add_no_overflow_loop
  ...
  := do
  loop
    (fun (x1, i1) => add_no_overflow_loop.body y x1 i1)
    (x, i)
...
/-- [tutorial::add_with_carry]: loop body 0:
    Source: 'src/lib.rs', lines 190:4-197:5
    Visibility: public -/
@[rust_loop_body]
def add_with_carry_loop.body
...
def add_with_carry_loop
  ...
  := do
  loop
    (fun (x1, c01, i1) => add_with_carry_loop.body y x1 c01 i1)
    (x, c0, i)
...
/-- [tutorial::add]: loop body 0:
    Source: 'src/lib.rs', lines 221:4-229:5
    Visibility: public -/
@[rust_loop_body]
def add_loop.body
  (y : alloc.vec.Vec Std.U32) (max1 : Std.Usize) (x : alloc.vec.Vec Std.U32)
...
def add_loop
  ...
  := do
  loop
    (fun (x1, c01, i1) => add_loop.body y max1 x1 c01 i1)
    (x, c0, i)
...
def pseudo_random_loop.body
  {T : Type} (HashInst : Hash T) (state : Std.U32) :
...
def pseudo_random_loop
  ...
  loop
    (fun state1 => pseudo_random_loop.body HashInst state1)
    state
